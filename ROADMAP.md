...
- [x] LGPL reading: the maintainer decided (2026-09-25) not to seek legal confirmation; the policy stands as written (`docs/DISTRIBUTION-POLICY.md` §3.4: every binary is distributed with its notices, the LGPL texts and its complete sources, and anyone who needs certainty obtains their own advice). Correction of the same day: the LGPL crate `cabac` reaches **every** build through `preflate-rs`, not only the full build through Lepton (`THIRD-PARTY-NOTICES.md`); removing it from every build is item 1.1 below
...
- [x] **network limits** (**done 2026-09-25**: `serve --revoke --allow-set --allow-file --max-bandwidth-kib --max-peers`, `fetch --revoke`; `docs/design/VOLUME-TRUST.md` §6.3-§6.4, `CHANGELOG.md`): a revocation file for pinned identities (`--revoke FILE`, checked before the handshake), a global bandwidth cap and a cap on distinct client addresses for `serve`, per-set `--allow` lists
...
- [ ] **one build**: once `cabac` is out (1.1), the only reason for the lite build is the size of the Lepton code; evaluate keeping Lepton in every build, or a permissively licensed lossless JPEG recompressor (JPEG XL's JPEG transcoding in libjxl, BSD-3; brunsli, Apache-2.0) behind a `requires` marker, so that the full/lite split disappears and every archive opens in every build
