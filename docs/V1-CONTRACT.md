...
| P8 | **Two builds, one format.** The full build (Lepton JPEG recompression, LGPL-3.0 component) and the lite build (`--no-default-features`, no Lepton; both builds contain the LGPL-3.0 component `cabac` through `preflate-rs`, see `THIRD-PARTY-NOTICES.md`) write the same format; `pack --no-lepton` makes the full build write archives every build can read; an archive reports `requires: lepton` when it needs the full build. | `docs/DISTRIBUTION-POLICY.md`, `tests/golden.rs` |
