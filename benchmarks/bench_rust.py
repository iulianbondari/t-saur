...
a shorter variant list when they exist.
"""
...
def pack(out: Path, src: Path, extra: list[str], keyfile: Path) -> tuple[dict, float]:
...
def main() -> None:
    ...
    lines = [f"# Rust reference implementation — benchmark (generated by `benchmarks/bench_rust.py`)\n",
             f"`{version}`, release build, single run per variant; times include process start-up. Corpora from `build_corpus.py` (and `build_extra_corpora.py` for the optional ones). Percentages are archive size relative to the input: **lower is better**.\n"]
    ...
    for corpus_name in corpora:
        ...
        lines.append("| Variant | Size | % of original | Ratio | Chunks (total/unique) | Blobs | Containers exploded/fallback | Codecs / filters | Pack (s) | Verify (s) | Unpack (s) | Round trip |")
        lines.append("|---|---:|---:|---:|---|---:|---|---|---:|---:|---:|---|")
        ...
        for i, (label, extra) in enumerate(variants):
            ...
            rep, t_pack = pack(out, src, extra, keyfile)
            ...
            lines.append(
                ...
                f"{rep['containers_exploded']}/{rep['containers_fallback']} | {' '.join(f'{k}:{v}' for k, v in sorted(rep['codec_hist'].items()))} | {t_pack:.2f} | {t_verify:.2f} | {t_unpack:.2f} | {'bit-exact OK' if ok else 'FAILED'} |"
            )
